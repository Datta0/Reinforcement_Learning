...
def trainer(epochs=1000, MINIBATCH_SIZE=40, GAMMA=0.99, epsilon=1.0, min_epsilon=0.01, BUFFER_SIZE=10000, train_indicator=True, render=False):
    with tf.Session() as sess:
        ...
        actor = ActorNetwork(sess, state_dim, action_dim,
                             action_bound, ACTOR_LEARNING_RATE, TAU, DEVICE)
        critic = CriticNetwork(sess, state_dim, action_dim, CRITIC_LEARNING_RATE,
                               TAU, actor.get_num_trainable_vars(), DEVICE)
        ...
        for i in range(epochs):
            ...
            print('th', i+1, 'n steps', step, 'R:', round(ep_reward, 3), 'Pos',
                  round(epsilon, 3), 'Efficiency', round(100.*((goal)/(i+1.)), 3))
